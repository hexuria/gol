...
<!-- BEGIN gol override header. Everything below END is impeccable-rust vendored unmodified from
     hexuria/impeccable-rust@ff71ccc079a93df231e19a12d7cc99e1da16efe1 (skill/SKILL.md), body sha256 6b6deaf4f05e171ddf2683545d2c2381dd53bb4d54e0c133c4ac404e09892dcd. -->
> **In gol, the AGENTS.md "Verification" section and `.claude/skills/gol-verify` win over this skill.** Load gol-verify with this skill whenever a trigger other than T0 fires. These rules are overridden; everything else applies as written.
>
> 1. Operating rule 3 ("Prefer automation that catches human misses (Miri, Loom, Kani, …)"): add a tool only when an AGENTS.md trigger requires it.
> 2. §2 Chaos ("Add at least one chaos layer"): gol has no default chaos layer.
> 3. §3 ("Kani … on high-risk logic"): Kani only under trigger T7.
> 4. §4 Benchmarks ("CI should fail on regression"), the Risk-to-owner row "Performance regression → Benchmark gate", and the anti-pattern about wall-clock microbenchmarks: Criterion is a measurement under T11, never a gate.
> 5. The Risk-to-owner table is replaced by the gol-verify ownership map.
> 6. Anti-drift ("copy the whole block into AGENTS.md"): do not copy it. "The system model" means only a model a gol trigger names (T2, T3, T10). Retry and cancel in `protocol::reduce` update `reduce_bounded`, not a model.
> 7. The "Verification impact" block and the "Verification" line of the review report: use the AGENTS.md PR declaration.
> 8. Modes: a model that AGENTS.md T2 or T3 requires is written in the same PR; neither audit mode's read-only first pass nor "recommend any new formal model instead of writing it" applies to it. Otherwise as written.
> 9. CI shape: replaced by the gol-verify charter. gol has no release gate, and every crate is `publish = false`, so the semver tools do not apply.
> 10. Terminology: gol adds "Bend-checked", "decided by evaluation", "design model (unlinked)", "Loom-checked" and "measured".
<!-- END gol override header -->
