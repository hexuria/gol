...
| T12 | paths: `AGENTS.md`, `CLAUDE.md`, `.claude/**`, `scripts/verify-*.sh`, `scripts/check-architecture.sh`, `scripts/ci-scope.sh`, `.github/**`, `deny.toml`, `.cargo/**`, `rust-toolchain{,.toml}`, `**/proptest-regressions/**`; lines± in `crates/**`: `^\s*#\[\s*(cfg_attr\(.*,\s*)?ignore\b`, `with_cases\(`, `\bcases\s*:\s*\d`, `cfg_attr\(miri`; lines± in `**/Cargo.toml`: `^\s*(doc\|auto)?tests?\s*=\s*false`; lines- in `crates/**`: `#\[(tokio::)?test\b`, `\bproptest!` | The owner applies the `verification:policy` label. List every removed or weakened check under `Not run:`. |
<!-- verify-plan:end -->
