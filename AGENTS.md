...
| T3 | lines± in `crates/*/src/**`: `(?i)\blease`, `(?i)heartbeat`, `(?i)keepalive`, `(?i)\bfenc`, `(?i)\bp?expire\b`, `(?i)\bset_?nx\b`, `(?i)\bnx\b`, `(?i)\bb?rpop\b`, `(?i)\bb?lmove\b`, `(?i)\brpoplpush\b`, `(?i)\bxreadgroup\b`, `(?i)\bxautoclaim\b`, `(?i)\bxack\b`, `(?i)\binsert into\b`, `(?i)\bon conflict\b`, `(?i)\bfor update\b`, `\bspawn\(`, `redis::`, `postgres::` | Flag: does this add or change a writer of shared persistent state? No: say why. Yes, and a model covers the resource: update it (T10). Yes, and none does: a new model in this PR (gol-verify "New model"). |
...
| T12 | paths: `AGENTS.md`, `CLAUDE.md`, `.claude/**`, `scripts/verify-plan.sh`, `scripts/check-architecture.sh`, `.github/**`, `deny.toml`, `.cargo/**`, `**/proptest-regressions/**`; lines± in `crates/**`: `#\[ignore`, `with_cases\(`, `cfg_attr\(miri` | The owner applies the `verification:policy` label. List every removed or weakened check under `Not run:`. |
<!-- verify-plan:end -->
...
- TLC on a config this PR changes exceeds 2 minutes or 1M distinct states.
