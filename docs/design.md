...
Non-normative. These are the harness design notes that used to be AGENTS.md sections 1 to 41: the preamble and the formal-methods essays on state machines, TLA+, Lean, retries, cancellation, idempotency, ownership and the rest.

They are guidance for design discussions only. They do not decide what a change must verify. The "Verification" section of `AGENTS.md` decides that, and it wins wherever these notes say "required", "must" or "model first". The `gol-verify` skill says how.
