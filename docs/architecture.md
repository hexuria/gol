...
Jev is the production decider. It calls System One; `cargo test` never calls the real System One, and the tests run `JevDecider` against a wiremock stand-in. `jev_state` shows Jev the input once in full, a summary of the fold, the last 16 event payloads with each string cut to 2048 characters, the tool catalog and the skills. `jev_choices` offers each catalog tool by name, `model` while model calls remain, and `complete`; once the step budget is spent it offers only `complete`, which while the harness is running is the one decision still free. An answer that was not offered is a decider error. `POST /v1/runs` uses `JevDecider`. Inside one `spawn_blocking` it stores the user message, runs the driver to the end, and appends the run's events to the stored log. A run is not left open when the decider or the queue push fails: a decider error appends the driver's events and then `RunFailed { Dependency }`, and a failed Redis push appends `RunFailed { Infrastructure }`; the response is still 502. `/v1/runs/{id}/ui` reads the dispatch phase when the harness never started, so such a run shows as failed. The work model stays on the spec and is not the decider.
